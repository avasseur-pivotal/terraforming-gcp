resource "google_dns_managed_zone" "env_dns_zone" {
  name        = "${var.env_name}-pas-zone"
  dns_name    = "${var.env_name_pas}.${var.dns_suffix}."
  description = "DNS zone for the ${var.env_name} environment"
}
resource "google_dns_managed_zone" "env_dns_zone_toplevel" {
  name        = "${var.env_name}-zone"
  dns_name    = "pcf.${var.dns_suffix}."
  description = "DNS zone for the ${var.env_name} environment"
}
resource "google_dns_managed_zone" "env_dns_zone_pks" {
  name        = "${var.env_name}-pks-zone"
  dns_name    = "${var.env_name_pks}.${var.dns_suffix}."
  description = "DNS zone for the ${var.env_name} environment"
}

resource "google_dns_record_set" "ops-manager-dns" {
  name = "opsman-pks.${google_dns_managed_zone.env_dns_zone_toplevel.dns_name}"
  type = "A"
  ttl  = 300
